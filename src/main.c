...
void I2C_GetData(char* message) {
    ...
    while (!(I2C1->SR1 & 1)) {} // Wait for start condition
    ...
    while (!(I2C1->SR1 & (1 << 7))) {} // Wait until data register is empty
    ...
    while (!(I2C1->SR1 & (1 << 2))) {} // wait until byte transfer is finished
    ...
    for (uint8_t i = 0; i < DATA_SIZE; i++) {
        while (!(I2C1->SR1 & (1 << 6))) {} // Wait until there is something in the receive register

        // Set NACK for the next-to-last byte (index 12)
        if (i == DATA_SIZE - 2) {
            I2C1->CR1 &= ~(1 << 10); // Clear ACK (Bit 10)
            I2C1->CR1 |= (1 << 9); // Set STOP (Bit 9)
        }
